...
#char is faster the varchar for fixed length data
...
insert into dogs(name,breed)
values('bob','retriever abcdefghijklmno');#this query get error because the breed varchar(10) and grt data is more than 10
...
select name,birthdate,day(birthdate) from people;#=> return the day

select name,birthdate,dayname(birthdate) from people;#=> gives the name of the day
...
select name,birthdt,date_add(birthdate,interval 1 QUARTER) from people;#here one QUARTER mens that the 3 months
